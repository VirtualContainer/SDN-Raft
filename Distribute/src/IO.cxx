...
asio_service::meta_cb_params req_to_params(RequestMessage* req, resp_msg* resp) {
...
public:
    rpc_session( uint64_t id,
        ...
        , crc_from_msg_(0)
    {
        output_trace("asio rpc session created: %p", this);
    }
...
public:
    ~rpc_session() {
        close_socket();
        output_trace("asio rpc session destroyed: %p", this);
    }
...
public:
    void prepare_handshake() {
        ...
        output_info( "session %" PRIu64 " got connection from %s:%u (as a server)",
              session_id_,
    ...
    void handle_handshake(ptr<rpc_session> self,
        ...
        if (!err) {
            output_info( "session %" PRIu64 " handshake with %s:%u succeeded (as a server)",
                  session_id_,
                  cached_address_.c_str(),
                  cached_port_ );
            this->start(self);
    ...
    void start(ptr<rpc_session> self) {
                  ...
                  (const ERROR_CODE& err, size_t) -> void
        {
            ...
            // Deprecate `Buffer::get` and use `BufferSerializer`.

            // header_->pos(0);
            BufferSerializer h_bs(header_);
            ...
            MessageType m_type = (MessageType)h_bs.get_u8();
...
private:
    ...
    void read_complete(ptr<Buffer> hdr, ptr<Buffer> log_ctx) {
       ...
       try {
        // Deprecate `Buffer::get` and use `BufferSerializer`.
        // hdr->pos(1);
        // MessageType t = (MessageType)hdr->get_byte();
        ...
        BufferSerializer h_bs(header_);
        h_bs.pos(1);
        MessageType t = (MessageType)h_bs.get_u8();
        ...
        if (!is_leader_) {
            ...
            if ( t == MessageType::append_entries_request ||
                 t == MessageType::sync_log_request ||
                 t == MessageType::join_cluster_request ||
                 t == MessageType::leave_cluster_request ||
                 t == MessageType::install_Snapshot_request ||
                 t == MessageType::priority_change_request ||
                 t == MessageType::custom_notification_request ) {
                is_leader_ = true;
                cb_func::ConnectionArgs
                    args( session_id_,
                          cached_address_,
        ...
        if (log_data_size > 0 && log_ctx) {
            ...
            while (log_ctx_size > ss.pos()) {
                if (log_ctx_size - ss.pos() < LOG_ENTRY_SIZE) {
                    ...
                    output_warn("wrong log ctx size %zu pos %zu, stop this session",
                         log_ctx_size, ss.pos());
                ...
                LogType val_type = (LogType)ss.get_u8();
                ...
                if (log_ctx_size - ss.pos() < val_size) {
                    // Out-of-bound size.
                    output_warn("wrong value size %zu log ctx %zu %zu, "
                         "stop this session",
                ...
                ss.get_buffer(buf);
                ptr<LogEntry> entry(
                    cs_new<LogEntry>(term, buf, val_type, timestamp, has_crc32, crc32, false) );
        ...
        if (!resp) {
            output_warn("no response is returned from raft message handler");
            this->stop();
            return;
        }
    ...
    void on_resp_ready(ptr<RequestMessage> req, ptr<resp_msg> resp) {
        ptr<rpc_session> self = this->shared_from_this();
       ...
       try {
        ...
        if (req->get_type() == MessageType::client_request ||
            req->get_type() == MessageType::add_server_request ||
            req->get_type() == MessageType::remove_server_request) {
...
public:
    asio_rpc_listener( asio_service_impl* _impl,
        ...
        , l_(l)
    {
        output_info("Raft ASIO listener initiated, %s",
...
private:
    ...
    {
        if (!err) {
            output_info("receive a incoming rpc connection");
...
public:
        ...
        , l_(l)
    {
        ...
        output_trace("asio client created: %p", this);
    }

    virtual ~asio_rpc_client() {
        output_trace("asio client destroyed: %p", this);
        close_socket();
    }
...
public:
                            ...
                            asio::ssl::verify_context& ctx)
    {
        if (impl_->get_options().verify_sn_) {
            ...
            output_debug("given subject: %s", subject_name);
                      ...
                      uint64_t send_timeout_ms = 0) __override__
    {
        ...
        while (!socket().is_open()) { // Dummy one-time loop
            output_debug( "socket %p to %s:%s is not opened yet",
                  this, host_.c_str(), port_.c_str() );
            ...
            if (!attempting_conn_.compare_exchange_strong(exp, desired)) {
                ...
                output_warn( "cannot send req as other thread is racing on opening "
            ...
            if (socket().is_open()) {
                ...
                output_warn("race: socket to %s:%s is already opened, escape",
                     host_.c_str(), port_.c_str());
                break;
            }
            ...
            if (impl_->get_options().custom_resolver_) {
                impl_->get_options().custom_resolver_(
                    ...
                    ( const std::string& resolved_host,
                      ...
                      std::error_code err ) {
                        if (!err) {
                            output_info( "custom resolver: %s:%s to %s:%s",
                                  host_.c_str(), port_.c_str(),
        ...
        if (ssl_enabled_ && !ssl_ready_) {
            ...
            output_warn( "cannot send req as SSL is not ready yet (%s:%s), count %zu",
                  host_.c_str(), port_.c_str(), num_send_fails_.load() );
            num_send_fails_.fetch_add(1);
        ...
        std::vector<ptr<Buffer>> LogEntry_bufs;
...
#endif
            LogEntry_bufs.push_back(entry_buf);
        ...
        // Deprecate `Buffer::put` and use `BufferSerializer`.
        ...
        BufferSerializer req_buf_bs(req_buf);
        ...
        {
            operation_timer_.expires_from_now
                   ( std::chrono::duration_cast<std::chrono::nanoseconds>
...
private:
    ...
    void set_busy_flag(bool to) {
        if (to == true) {
            ...
            if (!socket_busy_.compare_exchange_strong(exp, true)) {
                output_fetal("socket %p is already in use, race happened on connection to %s:%s",
                     this, host_.c_str(), port_.c_str());
                assert(0);
            }
        } else {
            ...
            if (!socket_busy_.compare_exchange_strong(exp, false)) {
                output_fetal("socket %p is already idle, race happened on connection to %s:%s",
                     this, host_.c_str(), port_.c_str());
                assert(0);
            }
        }
    ...
    void close_socket() {
        ...
        if (socket().is_open()) {
            ...
            if (l.owns_lock() && socket().is_open()) {
                socket().close();
                output_debug("socket to %s:%s closed", host_.c_str(), port_.c_str());
            } else {
                output_warn("race: socket to %s:%s is already closed",
                     host_.c_str(), port_.c_str());
            }
        }
#endif
    ...
    void cancel_socket(const ERROR_CODE& err) {
        ...
        if (socket().is_open()) {
            output_warn("cancelling operations due to socket (%s:%s) timeout",
                host_.c_str(), port_.c_str());
            abandoned_ = true;
            socket_.cancel();
        }
    ...
    void connected(ptr<RequestMessage>& req,
                   rpc_handler& when_done,
                   uint64_t send_timeout_ms,
                   std::error_code err,
                   asio::ip::tcp::resolver::iterator itor)
    {
        ...
        if (!err) {
            output_info( "%p connected to %s:%s (as a client)",
                  this, host_.c_str(), port_.c_str() );
            if (ssl_enabled_) {
#ifdef SSL_LIBRARY_NOT_FOUND
                assert(0); // Should not reach here.
    ...
    void handle_handshake(ptr<RequestMessage>& req,
                          rpc_handler& when_done,
                          uint64_t send_timeout_ms,
                          const ERROR_CODE& err)
    {
        ...
        if (!err) {
            output_info( "handshake with %s:%s succeeded (as a client)",
                  host_.c_str(), port_.c_str() );
            ssl_ready_ = true;
            this->send(req, when_done, send_timeout_ms);
    ...
    void sent( ptr<RequestMessage>& req,
               ptr<Buffer>& buf,
               rpc_handler& when_done,
               std::error_code err,
    ...
    void response_read(ptr<RequestMessage>& req,
                       rpc_handler& when_done,
                       ptr<Buffer>& resp_buf,
                       std::error_code err,
                       size_t bytes_transferred)
    {
        ...
        bs.pos(1);
        byte MessageType_val = bs.get_u8();
        ...
        ptr<resp_msg> rsp
            ( cs_new<resp_msg>
              ( term, (MessageType)MessageType_val, src, dst,
    ...
    void ctx_read(ptr<RequestMessage>& req,
                  ptr<resp_msg>& rsp,
                  rpc_handler& when_done,
                  ptr<Buffer>& ctx_buf,
                  uint32_t flags,
    ...
    bool handle_custom_resp_meta(ptr<RequestMessage>& req,
                                 ptr<resp_msg>& rsp,
                                 rpc_handler& when_done,
    ...
    , l_(l)
{
    if (my_opt_.enable_ssl_) {
        ...
        if (!_opt.ssl_context_provider_server_) {
            output_info("server SSL context method %d", DEFAULT_SERVER_CTX);
            ...
            output_info("custom server SSL context is given");
        }

        // Provider gives properly configured client contex
        if (!_opt.ssl_context_provider_client_) {
            output_info("client SSL context method %d", DEFAULT_CLIENT_CTX);

            // For client
            ssl_client_ctx_.load_verify_file(_opt.root_cert_file_);
        } else {
            output_info("custom client SSL context is given");
        }
#endif
    }
...
void asio_service_impl::worker_entry() {
    ...
    std::string thread_name = "SDN_Raft_w_" + std::to_string(worker_id);
#ifdef __linux__
    ...
    static Timer timer(60 * 1000000); // 1 min.
    ...
    do {
        ...
        if (timer.timeout_and_reset()) {
            ...
            output_fetal("too many exceptions (%zu) in 1-min time window.",
                 exception_count.load());
            exception_count = 0;
            abort();
        }
    ...
    output_info("end of asio worker thread, remaining threads: %u",
         num_active_workers_.load());
}
...
ptr<asio_service> SDN_Raft_global_mgr::init_asio_service
                  ( const asio_service_options& asio_opt,
                    ptr<Logger> Logger_inst )
{
    SDN_Raft_global_mgr* mgr = get_instance();
...
ptr<asio_service> SDN_Raft_global_mgr::get_asio_service() {
    // NOTE:

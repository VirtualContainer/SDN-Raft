...
void raft_server::commit(ulong target_idx) {
    if (target_idx > quick_commit_index_) {
        ...
        output_debug( "trigger commit upto %" PRIu64 "", quick_commit_index_.load() );
...
void raft_server::commit_in_bg() {
    std::string thread_name = "SDN_Raft_commit";
#ifdef __linux__
    ...
    while (true) {
     try {
        while ( quick_commit_index_ <= sm_commit_index_ ||
                sm_commit_index_ >= LogStore_->next_slot() - 1 ) {
            std::unique_lock<std::mutex> lock(commit_cv_lock_);
            ...
            auto wait_check = [this]() {
                ...
                return ( LogStore_->next_slot() - 1 > sm_commit_index_ &&
                         quick_commit_index_ > sm_commit_index_ );
            };
            output_trace("commit_cv_ sleep\n");
            commit_cv_.wait(lock, wait_check);

            output_trace("commit_cv_ wake up\n");
            if (stopping_) {
                lock.unlock();
                lock.release();
...
bool raft_server::commit_in_bg_exec(size_t timeout_ms) {
    ...
    output_debug( "commit upto %" PRIu64 ", curruent idx %" PRIu64,
          quick_commit_index_.load(), sm_commit_index_.load() );

    ulong log_start_idx = LogStore_->start_index();
    if ( log_start_idx &&
         sm_commit_index_ < log_start_idx - 1 ) {
        output_warn("current commit idx %" PRIu64 " is smaller than log start idx %" PRIu64 " - 1, "
             "adjust it to %" PRIu64 "",
    ...
    while ( sm_commit_index_ < quick_commit_index_ &&
        ...
        if (!first_loop_exec && timeout_ms && tt.timeout()) {
            output_warn( "abort commit due to timeout (%zu ms), %" PRIu64 " ms elapsed",
        ...
        output_trace( "commit upto %" PRIu64 ", current idx %" PRIu64 "\n",
              quick_commit_index_.load(), index_to_commit );

        ptr<LogEntry> le = LogStore_->entry_at(index_to_commit);
        if (!le)
        {
            // LCOV_EXCL_START
            output_fetal( "failed to get log entry with idx %" PRIu64 "", index_to_commit );
        ...
        if (le->get_term() == 0) {
            ...
            // (failed to read log).
            output_fetal( "empty log at idx %" PRIu64 ", must be log corruption",
                  index_to_commit );
            ctx_->state_mgr_->system_exit(raft_err::N19_bad_log_idx_for_term);
            ::exit(-1);
            // LCOV_EXCL_STOP
        ...
        if (le->get_val_type() == LogType::app_log) {
            commit_app_log(index_to_commit, le, need_to_handle_commit_elem);

        } else if (le->get_val_type() == LogType::conf) {
    ...
    output_debug( "DONE: commit upto %" PRIu64 ", curruent idx %" PRIu64,
          quick_commit_index_.load(), sm_commit_index_.load() );
    if (role_ == srv_role::follower) {
        ulong leader_idx = leader_commit_index_.load();
        ulong local_idx = sm_commit_index_.load();
                                 ...
                                 bool need_to_handle_commit_elem)
{
    ...
    if (pc_idx < sm_idx) {
        ...
        output_fetal( "pre-commit index %" PRIu64 " is smaller than commit index %" PRIu64,
              pc_idx, sm_idx );
    ...
    if (need_to_handle_commit_elem) {
        ...
        if (entry != commit_ret_elems_.end()) {
            ...
            if (elem->idx_ == sm_idx) {
                ...
                output_verbose("notify cb %" PRIu64 " %p", sm_idx, &elem->awaiter_);
        ...
        if (need_to_check_commit_ret) {
            ...
            elem->result_code_ = cmd_result_code::OK;
            elem->ret_value_ = ret_value;
            output_trace("commit thread is invoked earlier than user thread, "
...
void raft_server::commit_conf(ulong idx_to_commit,
    ...
    recur_lock(lock_);
    le->get_buf().pos(0);
    ptr<ClusterConfig> new_conf =
        ClusterConfig::deserialize(le->get_buf());

    ptr<ClusterConfig> cur_conf = get_config();
    output_info( "config at index %" PRIu64 " is committed, prev config log idx %" PRIu64 "",
    ...
    if (cur_conf->get_log_idx() < new_conf->get_log_idx()) {
        ...
        output_info( "skipped config %" PRIu64 ", latest config %" PRIu64 "",
              new_conf->get_log_idx(), cur_conf->get_log_idx() );
    }
    ...
    //     output_info("this server is committed as one of cluster members");
    //     catching_up_ = false;
    // }
}

bool raft_server::apply_config_LogEntry(ptr<LogEntry>& le,
                                         ptr<state_mgr>& s_mgr,
                                         std::string& err_msg)
{
    if (!le.get() || !s_mgr.get()) {
    ...
    if (le->get_val_type() != LogType::conf) {
    ...
    buf.pos(0);
    ptr<ClusterConfig> new_conf = ClusterConfig::deserialize(buf);
...
ulong raft_server::create_Snapshot() {
    ...
    output_info("manually create a Snapshot on %" PRIu64 "", committed_idx);
...
ptr< cmd_result<uint64_t> > raft_server::schedule_Snapshot_creation() {
    ...
    if (!snp_creation_scheduled_.compare_exchange_strong(exp, true)) {
        output_warn("Snapshot creation is already scheduled");
        return nilptr;
    }

    sched_snp_creation_result_ = cs_new<cmd_result<uint64_t>>();
    output_info("schedule Snapshot creation");
...
bool raft_server::Snapshot_and_compact(ulong committed_idx, bool forced_creation) {
    ...
    ptr<ClusterConfig> conf = get_config();
    if ( conf->get_prev_log_idx() >= LogStore_->next_slot() ) {
        // The latest config and previous config is not in LogStore,
    ...
    if (!forced_creation && !snp_creation_scheduled_) {
        ...
        if ( params->Snapshot_distance_ == 0 ||
             ( committed_idx - LogStore_->start_index() + 1 ) < Snapshot_distance ) {
 ...
 try {
    ...
    if (rc != CbReturnCode::Ok) {
        output_warn("creating a Snapshot %" PRIu64 " is rejected by user callback",
             committed_idx);
        return false;
    }
         ...
         snp_in_progress_.compare_exchange_strong(f, true) )
    {
        ...
        output_info("creating a Snapshot for index %" PRIu64 "", committed_idx);
        ...
        //
        //   To avoid such a case, while `snoutput_info_progress_` is true,
        ...
        if (local_snp && local_snp->get_last_log_idx() >= committed_idx) {
            output_warn("Snapshot index inversion detected, "
        ...
        if (snp_creation_scheduled_) {
            ...
            // Due to `snoutput_info_progress_` it will happen only once.
            manual_creation_cb = sched_snp_creation_result_;
            output_info("Snapshot creation is scheduled by user");
        }

        while ( conf->get_log_idx() > committed_idx &&
                conf->get_prev_log_idx() >= LogStore_->start_index() ) {
            ptr<LogEntry> conf_log
                ( LogStore_->entry_at( conf->get_prev_log_idx() ) );
            conf = ClusterConfig::deserialize(conf_log->get_buf());
        }
        ...
        if ( conf->get_log_idx() > committed_idx &&
            ...
            output_warn("config at log idx 1 is not availabe, "
        ...
        ulong log_term_to_compact = LogStore_->term_at(committed_idx);
        ptr<Snapshot> new_Snapshot
            ( cs_new<Snapshot>(committed_idx, log_term_to_compact, conf) );
        output_info( "create Snapshot idx %" PRIu64 " log_term %" PRIu64,
       ...
       ptr<std::exception>& err )
{
 do { // Dummy loop
    ...
    if (!result) {
        output_info("the state machine rejects to create the Snapshot");
        break;
    }

    {
        recur_lock(lock_);
        output_info("Snapshot idx %" PRIu64 " log_term %" PRIu64 " created, "
        ...
        if ( new_snp->get_last_log_idx() >
            ...
            ulong compact_upto = new_snp->get_last_log_idx() -
                                     (ulong)params->reserved_log_items_;
            output_info("LogStore_ compact upto %" PRIu64 "", compact_upto);
            ...
            LogStore_->compact_async(compact_upto, handler);
        }
    }
 } while (false);
...
void raft_server::reconfigure(const ptr<ClusterConfig>& new_config) {
    ptr<ClusterConfig> cur_config = get_config();
    output_info( "new config log idx %" PRIu64 ", prev log idx %" PRIu64 ", "
    ...
    output_debug( "system is reconfigured to have %zu servers, "
    ...
    for ( std::list<ptr<srv_config>>::const_iterator it = new_srvs.begin();
        ...
        if (id_ == (*it)->get_id()) {
            ...
            if (role_ == srv_role::follower &&
                ...
                output_info("now this node is the part of cluster, "
    ...
    for ( std::vector<ptr<srv_config>>::const_iterator it = srvs_added.begin();
        ...
        p->set_next_log_idx(LogStore_->next_slot());
        ...
        output_info("server %d is added to cluster", srv_added->get_id());
        if (role_ == srv_role::leader) {
            // Suppress following RPC error as it is expected.
            p->set_suppress_following_error();
            output_info("enable heartbeating for server %d", srv_added->get_id());
    ...
    for ( std::vector<int32>::const_iterator it = srvs_removed.begin();
        ...
        if (srv_removed == id_ && !catching_up_) {
            output_info("this server (%d) has been removed from the cluster, "
        ...
        if (pit != peers_.end()) {
            ...
            if (role_ == srv_role::leader && srv_to_leave_) {
                ...
                output_info("srv_to_leave_: %d", srv_to_leave_->get_id());
                ptr<Snapshot_sync_ctx> snp_ctx = srv_to_leave_->get_Snapshot_sync_ctx();
                if (snp_ctx) {
                    void* user_ctx = snp_ctx->get_user_snp_ctx();
                    output_info("srv_to_leave_ has Snapshot context %p and user context %p, "
                         "destroy them",
                ...
                if (!srv_to_leave_) {
                    output_info("srv_to_leave_ is currently empty "
            ...
            output_info("peer %d cannot be found, no action for removing", srv_removed);
        }
    }

    if (!str_buf.str().empty()) {
        output_info("%s", str_buf.str().c_str());
    }
    ...
    if ( uncommitted_config_ &&
        ...
        output_info("clearing uncommitted config at log %" PRIu64 ", prev %" PRIu64,
    ...
    output_info("new configuration: log idx %" PRIu64 ", prev log idx %" PRIu64 "\n"
...
void raft_server::remove_peer_from_peers(const ptr<peer>& pp) {
    output_info("server %d is removed from cluster", pp->get_id());
    pp->enable_hb(false);
...
void raft_server::pause_state_machine_exeuction(size_t timeout_ms) {
    output_info( "pause state machine execution, previously %s, state machine %s, "
          "timeout %zu ms",
    ...
    output_info( "waited %" PRIu64 " ms, state machine %s",
          timer.get_ms(),
          sm_commit_exec_in_progress_ ? "RUNNING" : "SLEEPING" );
}

void raft_server::resume_state_machine_execution() {
    output_info( "resume state machine execution, previously %s, state machine %s",
          sm_commit_paused_ ? "PAUSED" : "ACTIVE",
          sm_commit_exec_in_progress_ ? "RUNNING" : "SLEEPING" );

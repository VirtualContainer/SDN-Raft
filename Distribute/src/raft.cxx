...
raft_server::raft_server(context* ctx, const init_options& opt)
    ...
    , LogStore_(ctx->state_mgr_->load_LogStore())
    ...
    , test_mode_flag_(opt.test_mode_flag_)
{
    ...
    if (params->enable_randomized_Snapshot_creation_ &&
        ...
        uint64_t seed = Timer::get_timeofday_us() * id_;
        ...
        output_info("First Snapshot creation log distance %u", first_Snapshot_distance_);
    }

    apply_and_log_current_params();
    update_rand_timeout();
    precommit_index_ = LogStore_->next_slot() - 1;
    lagging_sm_target_index_ = LogStore_->next_slot() - 1;
    ...
    init_msg << "   === INIT RAFT SERVER ===\n"
             ...
             << "election timer " << ( state_->is_election_timer_allowed()
                                       ? "allowed" : "not allowed" ) << "\n"
             << "log store start " << LogStore_->start_index()
             << ", end " << LogStore_->next_slot() - 1 << "\n"
    ...
    output_info("%s", init_msg.str().c_str());
    ...
    for ( ulong i = std::max( sm_commit_index_ + 1,
          ...
          i < LogStore_->next_slot();
          ++i ) {
        auto const entry = LogStore_->entry_at(i);
        if (entry->get_val_type() == LogType::conf) {
            output_info( "detect a configuration change "
    ...
    output_info("%s", peer_info_msg.str().c_str());
...
void raft_server::start_server(bool skioutput_infoitial_election_timeout)
{
    ptr<raft_params> params = ctx_->get_params();
    global_mgr* mgr = get_global_mgr();
    if (mgr) {
        output_info("global manager is detected. will use shared thread pool");
        commit_bg_stopped_ = true;
        append_bg_stopped_ = true;
        mgr->init_raft_server(this);

    } else {
        output_info("global manager does not exist. "
    ...
    if (skioutput_infoitial_election_timeout) {
        // Issue #23:
        ...
        output_info("skip initialization of election timer by given parameter, "
        ...
        output_info("skip initialization of election timer by previously saved state, "
             "waiting for the first heartbeat");

    } else {
        output_info("wait for HB, for %d + [%d, %d] ms",
    ...
    output_debug("server %d started", id_);
}
...
void raft_server::update_rand_timeout() {
    ...
    output_info("new timeout range: %d -- %d",
         params->election_timeout_lower_bound_,
         params->election_timeout_upper_bound_);
}
...
void raft_server::apply_and_log_current_params() {
    ...
    if (!test_mode_flag_) {
        if (params->heart_beat_interval_ >= params->election_timeout_lower_bound_) {
            ...
            output_warn("invalid election timeout lower bound detected, adjusted to %d",
                 params->election_timeout_lower_bound_);
        }
        if (params->election_timeout_lower_bound_
            ...
            output_warn("invalid election timeout upper bound detected, adjusted to %d",
                 params->election_timeout_upper_bound_);
        }
    }
...
void raft_server::shutdown() {
    output_info("shutting down raft core");
    ...
    output_info("sent stop signal to the commit thread.");

    // Cancel all scheduler tasks.
    // TODO: how do we guarantee all tasks are done?
    cancel_schedulers();

    output_info("cancelled all schedulers.");
    ...
    output_info("commit thread stopped.");

    drop_all_pending_commit_elems();

    output_info("all pending commit elements dropped.");
    ...
    output_info("reset all pointers.");
    ...
    output_info("joined terminated commit thread.");
    ...
    output_info("sent stop signal to background append thread.");
...

    {
        ...
        output_info("clean up auto-forwarding queue: %zu elems", auto_fwd_reqs_.size());
        auto_fwd_reqs_.clear();
    }

    output_info("clean up auto-forwarding clients");
    cleanup_auto_fwd_pkgs();

    output_info("raft_server shutdown completed.");
}
...
ptr<resp_msg> raft_server::process_req(RequestMessage& req,
                                       const req_ext_params& ext_params) {
    cb_func::Param param(id_, leader_);
    param.ctx = &req;
    CbReturnCode rc = ctx_->cb_func_.call(cb_func::ProcessReq, &param);
    if (rc == CbReturnCode::ReturnNull) {
        output_warn("by callback, return null");
        return nullptr;
    }

    output_debug( "Receive a %s message from %d with LastLogIndex=%" PRIu64 ", "
          "LastLogTerm %" PRIu64 ", EntriesLength=%zu, CommitIndex=%" PRIu64 " and Term=%" PRIu64 "",
          MessageType_to_string(req.get_type()).c_str(),
          req.get_src(),
    ...
    if (stopping_) {
        ...
        output_warn("stopping, return null");
        return nullptr;
    }

    if ( req.get_type() == MessageType::client_request ) {
        // Client request doesn't need to go through below process.
        return handle_cli_req_prelock(req, ext_params);
    }

    recur_lock(lock_);
    if ( req.get_type() == MessageType::append_entries_request ||
         req.get_type() == MessageType::request_vote_request ||
         req.get_type() == MessageType::install_Snapshot_request ) {
    ...
    } else if (req.get_type() == MessageType::request_vote_request) {
        resp = handle_vote_req(req);

    } else if (req.get_type() == MessageType::pre_vote_request) {
        resp = handle_prevote_req(req);

    } else if (req.get_type() == MessageType::ping_request) {
        output_info("got ping from %d", req.get_src());
        resp = cs_new<resp_msg>( state_->get_term(),
                                 MessageType::ping_response,
                                 id_,
                                 req.get_src() );

    } else if (req.get_type() == MessageType::priority_change_request) {
    ...
    if (resp) {
        output_debug( "Response back a %s message to %d with Accepted=%d, "
              "Term=%" PRIu64 ", NextIndex=%" PRIu64 "",
              MessageType_to_string(resp->get_type()).c_str(),
              resp->get_dst(),
...
void raft_server::reset_peer_info() {
    ptr<ClusterConfig> c_config = get_config();
    auto const srv_cnt = c_config->get_servers().size();
    output_debug("servers: %zu\n", srv_cnt);
    if (srv_cnt > 1) {
        ...
        if (!my_srv_config) {
            ...
            // Just return.
            output_warn("my_srv_config is NULL");
            return;
        }

        ptr<ClusterConfig> my_next_config = cs_new<ClusterConfig>
        ...
        ptr<LogEntry> entry(cs_new<LogEntry>(
            state_->get_term(), new_conf_buf, LogType::conf,
            Timer::get_timeofday_us()));
        store_LogEntry(entry, LogStore_->next_slot()-1);
    }
}
...
void raft_server::handle_peer_resp(ptr<resp_msg>& resp, ptr<rpc_exception>& err) {
    ...
    if (err) {
        ...
        if (rpc_errs < raft_server::raft_limits_.warning_limit_) {
            output_warn("peer (%d) response error: %s", peer_id, err->what());
        } else if (rpc_errs == raft_server::raft_limits_.warning_limit_) {
            output_warn("too verbose RPC error on peer (%d), "
        ...
        if (pp && pp->is_leave_flag_set()) {
            ...
            handle_join_leave_rpc_err(MessageType::leave_cluster_request, pp);
        }
        return;
    }

    if (!resp.get()) {
        output_warn("empty peer response");
        return;
    }

    output_debug( "Receive a %s message from peer %d with "
    ...
    output_trace("src: %d, dst: %d, resp->get_term(): %d\n",
    ...
    if (resp->get_accepted()) {
        ...
        if (entry != peers_.end()) {
            ...
            if (rpc_errs >= raft_server::raft_limits_.warning_limit_) {
                output_warn("recovered from RPC failure from peer %d, %d errors",
    ...
    case MessageType::priority_change_response:
        handle_priority_change_resp(*resp);
        break;

    case MessageType::ping_response:
        output_info("got ping response from %d", resp->get_src());
        break;

    case MessageType::custom_notification_response:
...
void raft_server::send_reconnect_request() {
    ...
    if (entry != peers_.end()) {
        ...
        ptr<RequestMessage> req = cs_new<RequestMessage>( state_->get_term(),
                                            MessageType::reconnect_request,
                                            id_,
                                            leader_,
                                            0, 0, 0 );
        ...
        // LCOV_EXCL_START
        output_fetal("cannot find leader!");
        ctx_->state_mgr_->system_exit(N22_unrecoverable_isolation);
        // LCOV_EXCL_STOP
    }
}

ptr<resp_msg> raft_server::handle_reconnect_req(RequestMessage& req) {
    int32 srv_id = req.get_src();
    ptr<resp_msg> resp( cs_new<resp_msg>
                        ( state_->get_term(),
                          MessageType::reconnect_response,
                          id_,
                          srv_id ) );
    if (role_ != srv_role::leader) {
    ...
    output_info("re-connection to peer %d scheduled", srv_id);

    return resp;
}

void raft_server::handle_reconnect_resp(resp_msg& resp) {
    output_info("got re-connection scheduling response "
...
bool raft_server::reconnect_client(peer& p) {
    ...
    if (s_config) {
        output_debug( "reset RPC client for peer %d",
              p.get_id() );
...
void raft_server::become_leader() {
    ...
    {   auto_lock(commit_ret_elems_lock_);
        output_info("number of pending commit elements: %zu",
    ...
    {   auto_lock(cli_lock_);
        ...
        precommit_index_ = LogStore_->next_slot() - 1;
        output_info("state machine commit index %" PRIu64 ", "
        ...
        for (peer_itor it = peers_.begin(); it != peers_.end(); ++it) {
            ...
            pp->set_next_log_idx(LogStore_->next_slot());
            enable_hb_for_peer(*pp);
        }

        // If there are uncommitted logs, search if conf log exists.
        ptr<ClusterConfig> last_config = get_config();

        ulong s_idx = sm_commit_index_ + 1;
        ulong e_idx = LogStore_->next_slot();
        for (ulong ii = s_idx; ii < e_idx; ++ii) {
            ptr<LogEntry> le = LogStore_->entry_at(ii);
            if (le->get_val_type() != LogType::conf) continue;

            output_info("found uncommitted config at %" PRIu64 ", size %zu",
                 ii, le->get_buf().size());
            last_config = ClusterConfig::deserialize(le->get_buf());
        }

        // WARNING: WE SHOULD NOT CHANGE THE ORIGINAL CONTENTS DIRECTLY!
        ptr<ClusterConfig> last_config_cloned =
            ClusterConfig::deserialize( *last_config->serialize() );
        last_config_cloned->set_log_idx(LogStore_->next_slot());
        ...
        ptr<LogEntry> entry
            ( cs_new<LogEntry>
              ( state_->get_term(),
                ...
                LogType::conf,
                Timer::get_timeofday_us() ) );
        index_at_becoming_leader_ = store_LogEntry(entry);
        output_info("[BECOME LEADER] appended new config at %" PRIu64,
    ...
    if (my_priority_ == 0 && get_num_voting_members() > 1) {
        ...
        output_info("[BECOME LEADER] my priority is 0, will resign shortly");
        yield_leadership();
    }
}
...
bool raft_server::check_leadership_validity() {
    ...
    if ( (num_voting_members - nr_peers) < min_quorum_size ) {
        ...
        if (num_voting_members <= min_quorum_size) {
            output_warn("we cannot yield the leadership of this small cluster");
            return true;
        }
...
void raft_server::check_leadership_transfer() {
    ...
    if (!params->leadership_transfer_min_wait_time_) {
        ...
        output_trace("leadership transfer is disabled");
        return;
    }
    if (!leadership_transfer_timer_.timeout()) {
        // Leadership period is too short.
        output_trace("leadership period is too short: %zu ms",
    ...
    for (auto& entry: peers_) {
        ...
        if (peer_elem->get_matched_idx() + params->stale_log_gap_ <
            ...
            output_trace("peer %d is lagging behind, %lu < %lu",
        ...
        if (last_resp_ms > election_lower) {
            ...
            output_trace("peer %d is not responding, %lu ms ago",
    ...
    if (my_priority_ >= max_priority || successor_id == -1) {
        ...
        output_trace("my priority %d is already the highest", my_priority_);
        return;
    }
    ...
    if (!state_machine_->allow_leadership_transfer()) {
        ...
        output_trace("state machine does not allow leadership transfer");
        return;
    }

    output_info( "going to transfer leadership to %d, "
                                   ...
                                   int successor_id)
{
    ...
    if (cb_ret != cb_func::Ok) {
        output_info("[RESIGNATION REQUEST] refused by callback function");
        return;
    }
    ...
    if (immediate_yield) {
        output_info("got immediate re-elect request, resign now");
        leader_ = -1;
        become_follower();
    ...
    if (successor_id >= 0) {
        output_info("got graceful re-elect request (designated successor %d), "
             "pause write from now",
             successor_id);

        if ( successor_id >= 0 &&
             successor_id != candidate_id ) {
            output_warn("could not find given successor %d", successor_id);
        }

    } else {
        output_info("got graceful re-elect request, pause write from now");
    }

    if (candidate_id > -1) {
        output_info("next leader candidate: id %d endpoint %s priority %d "
             "last response %" PRIu64 " ms ago",
             candidate_id, candidate_endpoint.c_str(), max_priority,
             last_resp_ms);
        next_leader_candidate_ = candidate_id;;
    } else {
        output_warn("cannot find valid candidate for next leader, will proceed anyway");
    }
...
bool raft_server::request_leadership() {
    ...
    output_info("sent leadership request to leader %d", leader_.load());
    return true;
}

void raft_server::become_follower() {
    // stop hb for all peers
    output_info("[BECOME FOLLOWER] term %" PRIu64 "", state_->get_term());
    {   std::lock_guard<std::mutex> ll(cli_lock_);
        ...
        if ( params->auto_adjust_quorum_for_small_cluster_ &&
            ...
            output_warn("became 2-node cluster's follower, "
...
ptr<resp_msg> raft_server::handle_ext_msg(RequestMessage& req, std::unique_lock<std::recursive_mutex>& guard) {
    switch (req.get_type()) {
    case MessageType::add_server_request:
        return handle_add_srv_req(req);

    case MessageType::remove_server_request:
        return handle_rm_srv_req(req);

    case MessageType::sync_log_request:
        return handle_log_sync_req(req);

    case MessageType::join_cluster_request:
        return handle_join_cluster_req(req);

    case MessageType::leave_cluster_request:
    ...
    case MessageType::custom_notification_request:
...
void raft_server::handle_ext_resp(ptr<resp_msg>& resp, ptr<rpc_exception>& err) {
    ...
    output_debug("type: %d, err %p\n", (int)resp->get_type(), err.get());

    output_debug( "Receive an extended %s message from peer %d with Result=%d, "
          "Term=%" PRIu64 ", NextIndex=%" PRIu64 "",
          MessageType_to_string(resp->get_type()).c_str(),
          resp->get_src(),
          resp->get_accepted() ? 1 : 0,
    ...
    switch (resp->get_type())
    {
    case MessageType::sync_log_response:
        handle_log_sync_resp(*resp);
        break;

    case MessageType::join_cluster_response:
        handle_join_cluster_resp(*resp);
        break;

    case MessageType::leave_cluster_response:
...
void raft_server::handle_ext_resp_err(rpc_exception& err) {
    ptr<RequestMessage> req = err.req();
    output_info( "receive an rpc error response from peer server, %s %d",
          err.what(), req->get_type() );

    if ( req->get_type() == MessageType::install_Snapshot_request ) {
        if (srv_to_join_ && srv_to_join_->get_id() == req->get_dst()) {
            ...
            if (!timed_out) {
                ...
                output_warn("sending Snapshot to joining server %d failed, "
    ...
    if ( req->get_type() != MessageType::sync_log_request     &&
         req->get_type() != MessageType::join_cluster_request &&
         req->get_type() != MessageType::leave_cluster_request ) {
        return;
    }

    ptr<peer> p;
    MessageType t_msg = req->get_type();
    ...
    if (p->get_current_hb_interval() >= ctx_->get_params()->max_hb_interval()) {
        ...
        output_debug("retry the request");
        p->slow_down_hb();
...
void raft_server::on_retryable_req_err(ptr<peer>& p, ptr<RequestMessage>& req) {
    output_debug( "retry the request %s for %d",
          MessageType_to_string(req->get_type()).c_str(), p->get_id() );
    if (p->make_busy()) {
...
ptr<ClusterConfig> raft_server::get_config() const {
    std::lock_guard<std::mutex> l(config_lock_);
    ptr<ClusterConfig> ret = config_;
    return ret;
}

void raft_server::set_config(const ptr<ClusterConfig>& new_config) {
    std::lock_guard<std::mutex> l(config_lock_);
...
ulong raft_server::store_LogEntry(ptr<LogEntry>& entry, ulong index) {
    ulong log_index = index;
    if (index == 0) {
        log_index = LogStore_->append(entry);
    } else {
        LogStore_->write_at(log_index, entry);
    }
    ...
    if ( entry->get_val_type() == LogType::conf ) {
        ...
        if ( !LogStore_->flush() ) {
            // LCOV_EXCL_START
            output_fetal("log store flush failed");
...
global_mgr* raft_server::get_global_mgr() const {
    ...
    return SDN_Raft_global_mgr::get_instance();
}

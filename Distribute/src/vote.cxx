...
void raft_server::request_prevote() {
    ...
    for (peer_itor it = peers_.begin(); it != peers_.end(); ++it) {
        ...
        if (s_config) {
            ...
            if (recreate) {
                output_info("reset RPC client for peer %d", s_config->get_id());
    ...
    if (pre_vote_.live_ + pre_vote_.dead_ > 0) {
        if (pre_vote_.live_ + pre_vote_.dead_ < quorum_size + 1) {
            ...
            output_warn("total %d nodes (including this node) responded for pre-vote "
    ...
    if ( params->auto_adjust_quorum_for_small_cluster_ &&
        ...
        output_warn("2-node cluster's pre-vote is failing long time, "
             "adjust quorum to 1");
    ...
    if ( my_priority_ < target_priority_ ) {
        if ( check_cond_for_zp_election() ) {
            output_info("[PRIORITY] temporarily allow election for zero-priority member");
        } else {
            output_info("[PRIORITY] will not initiate pre-vote due to priority: "
    ...
    output_info("[PRE-VOTE INIT] my id %d, my role %s, term %" PRIu64 ", log idx %" PRIu64 ", "
         "log term %" PRIu64 ", priority (target %d / mine %d)\n",
         id_, srv_role_to_string(role_).c_str(),
         state_->get_term(), LogStore_->next_slot() - 1,
    ...
    for (peer_itor it = peers_.begin(); it != peers_.end(); ++it) {
        ...
        if (pp->make_busy()) {
            ...
            output_warn("failed to send prevote request: peer %d (%s) is busy",
...
void raft_server::initiate_vote(bool force_vote) {
    ...
    if ( !force_vote &&
        ...
        output_info("grace period option is enabled, and state machine needs catch-up: "
             "%" PRIu64 " vs. %" PRIu64 "",
             sm_commit_index_.load(),
             lagging_sm_target_index_.load());
        if (vote_init_timer_term_ != cur_term) {
            output_info("grace period: %d, term increment detected %" PRIu64 " vs. %" PRIu64
                 ", reset timer",
        ...
        if ( vote_init_timer_term_ == cur_term &&
            ...
            output_info("grace period: %d, term %" PRIu64 ", waited %" PRIu64
                 " ms, skip initiating vote",
                 grace_period, cur_term, vote_init_timer_.get_ms());
            return;

        } else {
            output_info("grace period: %d, no new leader detected for term %" PRIu64
                 " for %" PRIu64 " ms",
                 grace_period, cur_term, vote_init_timer_.get_ms());
        }
    }
...
void raft_server::request_vote(bool force_vote) {
    ...
    votes_granted_ += 1;
    votes_responded_ += 1;
    output_info("[VOTE INIT] my id %d, my role %s, term %" PRIu64 ", log idx %" PRIu64 ", "
         "log term %" PRIu64 ", priority (target %d / mine %d)\n",
         id_, srv_role_to_string(role_).c_str(),
         state_->get_term(), LogStore_->next_slot() - 1,
         term_for_log(LogStore_->next_slot() - 1),
    ...
    for (peer_itor it = peers_.begin(); it != peers_.end(); ++it) {
        ...
        if (force_vote) {
            ...
            // Force vote message, and wrap it using LogEntry.
            ptr<force_vote_msg> fv_msg = cs_new<force_vote_msg>();
            ptr<LogEntry> fv_msg_le =
                cs_new<LogEntry>(0, fv_msg->serialize(), LogType::custom);

            // Ship it.
            req->log_entries().push_back(fv_msg_le);
        }
        output_debug( "send %s to server %d with term %" PRIu64 "",
        ...
        if (pp->make_busy()) {
            ...
            output_warn("failed to send vote request: peer %d (%s) is busy",
                 pp->get_id(), pp->get_endpoint().c_str());
        }
    }
}

ptr<resp_msg> raft_server::handle_vote_req(RequestMessage& req) {
    output_info("[VOTE REQ] my role %s, from peer %d, "
         ...
         req.get_src(), req.get_last_log_term(), LogStore_->last_entry()->get_term(),
    ...
    bool log_okay =
        req.get_last_log_term() > LogStore_->last_entry()->get_term() ||
        ( req.get_last_log_term() == LogStore_->last_entry()->get_term() &&
          LogStore_->next_slot() - 1 <= req.get_last_log_idx() );
    ...
    if (req.log_entries().size() > 0) {
        output_info("[VOTE REQ] force vote request, will ignore priority");
        ignore_priority = true;
    }
    if (catching_up_) {
        output_info("[VOTE REQ] this server is catching-up with leader, "
    ...
    if (grant) {
        ...
        for (auto& entry: c_conf->get_servers()) {
            ...
            if ( !ignore_priority &&
                ...
                //   priority.
                output_info("I (%d) could vote for peer %d, "
                     "but priority %d is lower than %d",
                     id_, s_conf->get_id(),
                     s_conf->get_priority(), target_priority_);
                output_info("decision: X (deny)\n");
                return resp;
            }
        }

        output_info("decision: O (grant), voted_for %d, term %" PRIu64,
             req.get_src(), resp->get_term());
        resp->accept(LogStore_->next_slot());
        state_->set_voted_for(req.get_src());
        ctx_->state_mgr_->save_state(*state_);
    } else {
        output_info("decision: X (deny), term %" PRIu64, resp->get_term());
    }
...
void raft_server::handle_vote_resp(resp_msg& resp) {
    if (election_completed_) {
        output_info("Election completed, will ignore the voting result from this server");
        return;
    }

    if (resp.get_term() != state_->get_term()) {
        // Vote response for other term. Should ignore it.
        output_info("[VOTE RESP] from peer %d, my role %s, "
    ...
    output_info("[VOTE RESP] peer %d (%s), resp term %" PRIu64 ", my role %s, "
    ...
    if (votes_granted_ >= election_quorum_size) {
        output_info("Server is elected as leader for term %" PRIu64, state_->get_term());
        election_completed_ = true;
        become_leader();
        output_info("  === LEADER (term %" PRIu64 ") ===\n", state_->get_term());
    }
}

ptr<resp_msg> raft_server::handle_prevote_req(RequestMessage& req) {
    ...
    output_info("[PRE-VOTE REQ] my role %s, from peer %d, "
         "log term: req %" PRIu64 " / mine %" PRIu64 "\n"
         "last idx: req %" PRIu64 " / mine %" PRIu64
         ", term: req %" PRIu64 " / mine %" PRIu64 "\n"
         "%s",
    ...
    //   requests.
    if (catching_up_) {
        output_info("this server is catching up, always accept pre-vote");
    }
    if (!hb_alive_ || catching_up_) {
        output_info("pre-vote decision: O (grant)");
        resp->accept(LogStore_->next_slot());
    } else {
        if (next_idx_for_resp != std::numeric_limits<ulong>::max()) {
            output_info("pre-vote decision: X (deny)");
        } else {
            output_info("pre-vote decision: XX (strong deny, non-existing node)");
        }
    }
...
void raft_server::handle_prevote_resp(resp_msg& resp) {
    if (resp.get_term() != pre_vote_.term_) {
        // Vote response for other term. Should ignore it.
        output_info("[PRE-VOTE RESP] from peer %d, my role %s, "
    ...
    output_info("[PRE-VOTE RESP] peer %d (%s), term %" PRIu64 ", resp term %" PRIu64 ", "
    ...
    if (pre_vote_.dead_ >= election_quorum_size) {
        output_info("[PRE-VOTE DONE] SUCCESS, term %" PRIu64, pre_vote_.term_);

        bool exp = false;
        bool val = true;
        if (pre_vote_.done_.compare_exchange_strong(exp, val)) {
            output_info("[PRE-VOTE DONE] initiate actual vote");

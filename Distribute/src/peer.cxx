                     ...
                     rpc_handler& handler )
{
    ...
    if (req) {
        output_trace("send req %d -> %d, type %s",
             req->get_src(),
             req->get_dst(),
    ...
    {   std::lock_guard<std::mutex> l(rpc_protector_);
        if (!rpc_) {
            ...
            output_trace("rpc local is null");
            set_free();
            return;
        }
        rpc_local = rpc_;
...
{
    const static std::unordered_set<int> MessageTypes_to_free( {
    ...
    if (abandoned_) {
        output_info("peer %d has been shut down, ignore response.", config_->get_id());
        return;
    }

    if (req) {
        output_trace( "resp of req %d -> %d, type %s, %s",
              req->get_src(),
              req->get_dst(),
              MessageType_to_string( req->get_type() ).c_str(),
              (err) ? err->what() : "OK" );
    ...
    if (err == nilptr) {
        ...
        {   std::lock_guard<std::mutex> l(rpc_protector_);
            ...
            if (cur_rpc_id != given_rpc_id) {
                output_warn( "[EDGE CASE] got stale RPC response from %d: "
            ...
            if ( MessageTypes_to_free.find(req->get_type()) != MessageTypes_to_free.end() ) {
                set_free();
            }
        }
        ...
        {   std::lock_guard<std::mutex> l(rpc_protector_);
            ...
            if (cur_rpc_id == given_rpc_id) {
                rpc_.reset();
                if ( MessageTypes_to_free.find(req->get_type()) !=
                         MessageTypes_to_free.end() ) {
                    set_free();
                }
                ...
                output_warn( "[EDGE CASE] RPC for %d has been reset before "
...
{
    if (abandoned_) {
        output_trace("peer %d is abandoned", config->get_id());
        return false;
    }
    ...
    if (!factory) {
        output_trace("client factory is empty");
        return false;
    }
    ...
    if (backoff_timer_disabled) {
        output_trace("reconnection back-off timer is disabled");
    }
    ...
    if (backoff_timer_disabled || reconn_backoff_.timeout()) {
        ...
        output_trace("%p reconnect peer %d", rpc_.get(), config_->get_id());
        ...
        output_trace("skip reconnect this time");
    }
    return false;
}

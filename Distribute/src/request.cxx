...
ptr<resp_msg> raft_server::handle_cli_req_prelock(RequestMessage& req,
                                                  const req_ext_params& ext_params)
{
    ptr<resp_msg> resp = nullptr;
    ptr<raft_params> params = ctx_->get_params();
    uint64_t timestamp_us = Timer::get_timeofday_us();
...
void raft_server::request_append_entries_for_all() {
    ...
    if (params->use_bg_thread_for_urgent_commit_) {
        ...
        if (mgr) {
            ...
            output_trace("found global thread pool");
...
ptr<resp_msg> raft_server::handle_cli_req(RequestMessage& req,
                                          const req_ext_params& ext_params,
                                          uint64_t timestamp_us)
{
    ...
    for (size_t i = 0; i < num_entries; ++i) {
        ...
        ulong next_slot = store_LogEntry(entries.at(i));
        output_debug("append at log_idx %" PRIu64 ", timestamp %" PRIu64,
    ...
    if (num_entries) {
        LogStore_->end_of_append_batch(last_idx - num_entries, num_entries);
    }
    try_update_precommit_index(last_idx);
    resp_idx = LogStore_->next_slot();
    ...
    if (!get_config()->is_async_replication()) {
        ...
        {   auto_lock(commit_ret_elems_lock_);
            ...
            if (entry != commit_ret_elems_.end()) {
                ...
                elem = entry->second;
                output_trace("commit thread was faster than this thread: %p", elem.get());
            } else {
        ...
        output_verbose( "asynchronously replicated %" PRIu64 ", return value %p",
...
ptr<resp_msg> raft_server::handle_cli_req_callback(ptr<commit_ret_elem> elem,
                                                   ptr<resp_msg> resp) {
    output_verbose("commit_ret_cv %" PRIu64 " %p sleep", elem->idx_, &elem->awaiter_);
    ...
    {   auto_lock(commit_ret_elems_lock_);
        ...
        if (elem->result_code_ != cmd_result_code::TIMEOUT) {
            ...
            output_verbose("Client timeout leave commit thread to remove commit_ret_elem %" PRIu64,
                 idx);
        }
        output_verbose("remaining elems in waiting queue: %zu", commit_ret_elems_.size());
    }

    if (elem->result_code_ == cmd_result_code::OK) {
        output_verbose( "[OK] commit_ret_cv %" PRIu64 " wake up (%" PRIu64 " us), return value %p",
              idx, elapsed_us, ret_value.get() );
    } else {
        // Null `ret_value`, most likely timeout.
        output_warn( "[NOT OK] commit_ret_cv %" PRIu64 " wake up (%" PRIu64 " us), "
              "return value %p, result code %d",
              idx, elapsed_us, ret_value.get(), elem->result_code_ );
        bool valid_leader = check_leadership_validity();
        if (valid_leader) {
            output_info("leadership is still valid");
        } else {
...
void raft_server::drop_all_pending_commit_elems() {
    ...
    if (ctx_->get_params()->return_method_ == raft_params::blocking) {
        ...
        for (auto& entry: commit_ret_elems_) {
            ...
            output_debug("cancelled blocking client request %" PRIu64 ", waited %" PRIu64 " us",
                 elem->idx_, elem->timer_.get_us());
        }
        if (!commit_ret_elems_.empty()) {
            output_warn("cancelled %zu blocking client requests from %" PRIu64
                 " to %" PRIu64 ".",
    ...
    for (auto& entry: elems) {
        ...
        output_warn("cancelled non-blocking client request %" PRIu64, ee->idx_);

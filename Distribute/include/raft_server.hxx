...
class raft_server : public std::enable_shared_from_this<raft_server> {
    friend class SDN_Raft_global_mgr;
...
public:
    ...
    { return LogStore_->term_at(get_last_log_idx()); }
     ...
     */
    ptr<ClusterConfig> get_config() const;
     ...
     */
    ptr<LogStore> get_LogStore() const { return LogStore_; }
     ...
     * @return `true` on success.
     */
    static bool apply_config_LogEntry(ptr<LogEntry>& le,
                                       ptr<state_mgr>& s_mgr,
                                       std::string& err_msg);
...
protected:
     ...
     * @return Response.
     */
    virtual ptr<resp_msg> process_req(RequestMessage& req, const req_ext_params& ext_params);
    ...
    ptr<resp_msg> handle_cli_req_prelock(RequestMessage& req, const req_ext_params& ext_params);
    ptr<resp_msg> handle_cli_req(RequestMessage& req,
                                 const req_ext_params& ext_params,
    ...
    ptr<resp_msg> handle_append_entries(RequestMessage& req);
    ptr<resp_msg> handle_prevote_req(RequestMessage& req);
    ptr<resp_msg> handle_vote_req(RequestMessage& req);
    ptr<resp_msg> handle_ext_msg(RequestMessage& req, std::unique_lock<std::recursive_mutex>& guard);
    ptr<resp_msg> handle_install_Snapshot_req(RequestMessage& req, std::unique_lock<std::recursive_mutex>& guard);
    ptr<resp_msg> handle_rm_srv_req(RequestMessage& req);
    ptr<resp_msg> handle_add_srv_req(RequestMessage& req);
    ptr<resp_msg> handle_log_sync_req(RequestMessage& req);

    ptr<resp_msg> handle_join_cluster_req(RequestMessage& req);
    ptr<resp_msg> handle_leave_cluster_req(RequestMessage& req);
    
    ptr<resp_msg> handle_priority_change_req(RequestMessage& req);
    ptr<resp_msg> handle_reconnect_req(RequestMessage& req);
    ptr<resp_msg> handle_custom_notification_req(RequestMessage& req);
    ...
    void handle_join_leave_rpc_err(MessageType t_msg, ptr<peer> p);
    void reset_srv_to_join();
    void reset_srv_to_leave();
    ptr<RequestMessage> create_append_entries_req(ptr<peer>& pp);
    ptr<RequestMessage> create_sync_Snapshot_req(ptr<peer>& pp,
                                          ulong last_log_idx,
                                          ulong term,
                                          ulong commit_idx,
                                          bool& succeeded_out);
    ...
    void reconfigure(const ptr<ClusterConfig>& new_config);
    ...
    void on_log_compacted(ulong log_idx,
                          bool result,
                          ptr<std::exception>& err);
    void on_retryable_req_err(ptr<peer>& p, ptr<RequestMessage>& req);
    ...
    void commit_conf(ulong idx_to_commit, ptr<LogEntry>& le);

    ptr< cmd_result< ptr<Buffer> > >
        send_msg_to_leader(ptr<RequestMessage>& req,
    ...
    void set_config(const ptr<ClusterConfig>& new_config);
    ptr<Snapshot> get_last_Snapshot() const;
    void set_last_Snapshot(const ptr<Snapshot>& new_Snapshot);

    ulong store_LogEntry(ptr<LogEntry>& entry, ulong index = 0);

    ptr<resp_msg> handle_out_of_log_msg(RequestMessage& req,
                                        ptr<custom_notification_msg> msg,
                                        ptr<resp_msg> resp);

    ptr<resp_msg> handle_leadership_takeover(RequestMessage& req,
                                             ptr<custom_notification_msg> msg,
                                             ptr<resp_msg> resp);

    ptr<resp_msg> handle_resignation_request(RequestMessage& req,
                                             ptr<custom_notification_msg> msg,
                                             ptr<resp_msg> resp);
...
protected:
     ...
     */
    Timer last_election_timer_reset_;
     ...
     */
    ptr<ClusterConfig> uncommitted_config_;
